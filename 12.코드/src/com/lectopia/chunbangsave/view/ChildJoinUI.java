...
public class ChildJoinUI extends JDialog 
{
	...
	public ChildJoinUI(JFrame frame) 
	{
		super(frame, true);
		setTitle("아이 회원가입");
		...
		joinBtn.addActionListener(new ChildJoinBtnListener());
		checkIDBtn.addActionListener(new ChildMemberCheckBtnListener());
		checkParentIDBtn.addActionListener(new ParentAcceptBtnListener());
		/*
		 * 콤보박스 날짜 만들기 
		 */
	...
	/**
	 * 자녀 아이디 중복 체크 버튼 이벤트 핸들러
	 * @author yeeun
	 *
	 */
	private class ChildMemberCheckBtnListener implements ActionListener
	{
	...
	/**
	 * 부모님 아이디 확인 버튼 이벤트 핸들러 
	 * @author yeeun
	 *
	 */
	private class ParentAcceptBtnListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e) 
		{
			/**** 넘겨줄 데이터 : 부모님 아이디 
			 *  가져올 데이터 : 가족번호 (존재하지 않는 아이디일 경우 -1) *****/
		}
	}
	/**
	 * 자녀 회원 가입 버튼 이벤트 핸들러 
	 ...
	 *
	 */
	private class ChildJoinBtnListener implements ActionListener
	{
		public void actionPerformed(ActionEvent e) 
		{
